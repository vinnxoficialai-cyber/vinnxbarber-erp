...
/* ═══════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════ */
function DropdownItem({ label, options, isDark }) {
...
/* ═══════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════ */
function ModalButton({ label, maxW, hasTabs, isSplit }) {
  const [open, setOpen] = useState(false);
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          <Card>
            <div style={{ display: "flex", flexDirection: "column", gap: "2.5rem" }}>
              {users.map((u, i) => (<div key={i} style={{ display: "flex", gap: "1rem", alignItems: "flex-start" }}><div style={{ position: "relative", flexShrink: 0 }}><div style={{ width: 40, height: 40, borderRadius: "9999px", background: T.bgPage, display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ fontSize: "1rem", fontWeight: 500, color: T.textHeading, textTransform: "uppercase" }}>{u.initial}</span></div>{u.showStatus && <div style={{ position: "absolute", top: 0, right: 0, width: 13, height: 13, borderRadius: "9999px", background: T.Dot, border: `2px solid ${T.border}`, transform: "translate(15%,-15%)" }} />}</div><div style={{ display: "flex", flexDirection: "column", gap: "0.25rem", minHeight: 40, justifyContent: "center" }}><span style={{ fontSize: "1rem", fontWeight: 600, color: T.textHeading, lineHeight: 1.3 }}>{u.name}</span>{u.email && <span style={{ fontSize: "0.875rem", fontWeight: 400, color: T.textHeading, lineHeight: 1.3 }}>{u.email}</span>}{u.roles.length > 0 && <div style={{ display: "flex", gap: "0.5rem", marginTop: "0.25rem" }}>{u.roles.map((r, j) => <span key={j} style={{ background: T.bgPage, borderRadius: "9999px", padding: "2px 10px", fontSize: "0.7rem", fontWeight: 600, color: T.textHeading, lineHeight: 1.6 }}>{r}</span>)}</div>}</div></div>))}
            </div>
          </Card>
